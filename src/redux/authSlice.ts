...
import { AuthData, ProfileType, SignInPayload } from '../types/types';
...
>('getCaptcha', async (_, { rejectWithValue }) => {
  try {
    return await securityAPI.getCaptchaUrl();
  } catch (e) {
...
>(
  ...
  async (
    ...
    try {
      const signInData = await authAPI.login(
        email,
        password,
        rememberMe,
        captcha
      ...
      if (signInData.resultCode === ResultCodes.success) {
        ...
        signInData.resultCode === CaptchaResultCodes.captchaIsRequired
      ) {
        dispatch(getCaptcha());
